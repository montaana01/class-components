  ...
  return (
    <>
      <tr
        ...
        style={{ cursor: 'pointer', background: expanded ? 'rgb(133 106 106)' : 'inherit' }}
      >
        <td>{name}</td>
        ...
        {extraColumns.map((col) => (
          <td key={col}>{lastYearData?.[col as keyof typeof lastYearData] ?? notFound}</td>
        ))}
      </tr>
      {expanded && (
        <tr>
              ...
              <table className={styles.detailTable}>
                ...
                {countryObj.data.map((entry) => (
                  <tr key={entry.year}>
                    ...
                    {extraColumns.map((col) => (
                      <td key={col}>{entry[col as keyof typeof entry] ?? notFound}</td>
                    ))}
                  </tr>
                ))}
                </tbody>
